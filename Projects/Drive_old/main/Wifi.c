...
int STATIC_IP_FLAG = 1;  // flag to set static ip mode on 
char url[256] = { 0 }; 
char special_string[SPECIAL_STRING_LENGTH] = "special";
char SYSTEM_DEBUG_MSG[50]                  = {"NONE"}; //debug message
char DEVICE_IP[]                           = "192.168.0.69";
const static char DEVICE_GW[]              = "192.168.0.1";
    ...
    if(semaphore_taken == pdTRUE)
    {
        for (int i = 0; i < MAX_CONNECTIONS; ++i)
        {
            if (clients[i] == NULL)
            {
                ...
                clients[i] = conn;
                printf("Added SSE to zndvkjbl Channel (%d)\n", i);
                success = true;
                break;
            }
        }
