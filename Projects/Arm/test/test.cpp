...
FAKE_VOID_FUNC(gpio_set_level, gpio_num_t, uint32_t);
FAKE_VALUE_FUNC(int, gpio_get_level, gpio_num_t);

FAKE_VOID_FUNC(ledc_update_duty, ledc_mode_t, ledc_channel_t);
FAKE_VOID_FUNC(ledc_set_duty, ledc_mode_t, ledc_channel_t, uint32_t);

FAKE_VALUE_FUNC(uint32_t, esp_log_timestamp);

FAKE_VOID_FUNC(gpio_set_direction, gpio_num_t, gpio_mode_t);
...
TEST_CASE( "Testing Unittest", "[test]" )
{
	...
	SECTION( "Testing Send Data", "[send]" )
	{
		...
		send_data.data[send_rotunda] = 100;
		send_data.data[send_shoulder] = 200;
		send_data_wifi(2);
